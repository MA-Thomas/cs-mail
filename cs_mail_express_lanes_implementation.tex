...
that the \textit{cs-mail Protocol Specification} advertises at launch. It extends the
...
\begin{enumerate}[leftmargin=2em]
    ...
    message declarations match its constraints, deliver bond-free through the lane
    path;
...
allowance, and moves no value. The message may use the ordinary bonded path only
after obtaining and satisfying its own valid contact terms; the service does not
silently reinterpret an unbonded lane request as a bonded attempt. Admission after
message validity returns \texttt{MessageValidityClosed} under the ordinary full-
release rules for any still-unadmitted reservation.
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Authenticated declaration enforcement.} Replace the existing
    display-only purpose label with constrained declarations, bind them to native
    content and bond-free admission signing bytes, add explicit gateway authority,
...
Authenticated declaration enforcement is an additive correction to the admission
edge: the earlier implementation enforced identity, rate, and lifetime while
treating purpose as display-only. The follow-on makes the specification's existing
``matching capability'' language concrete without claiming provider knowledge of
encrypted content and without moving capability logic into settlement.

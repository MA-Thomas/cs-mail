...
Environment-gated provider harnesses do not establish that real OIDC/bank providers were exercised. This is a historical validation record. Current product login and recovery scope is defined in [the product identity model](product-identity-model.md); Phoros ceremony-bound subject adoption remains deferred. No production hosting was configured. Before committing cs-mail, commit the matching identity-model revision and keep `identity-source.sha256` in sync with it.

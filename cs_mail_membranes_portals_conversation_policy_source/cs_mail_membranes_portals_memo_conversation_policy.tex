...
\hypersetup{
  ...
  pdfsubject={Conversation-level privacy and reuse policy revision}
}
...
\begin{itemize}[leftmargin=*]
    \item granting a sender standing permission creates a route through the membrane;
    ...
    \item an express lane creates a constrained passage, potentially limited by identity, purpose, duration, or message density.
\end{itemize}
...
The spatial distinction should remain semantically stable. ``Inside'' should consistently indicate some form of authorized relationship. ``Outside'' should consistently indicate that no standing route exists. Animation is useful when it explains a state transition, for example when accepting a first-contact request visibly opens a route. It should not exist merely to make the product feel futuristic.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Consequential actions must remain explicit.} Granting permission, revoking access, sharing content, changing a conversation's reuse policy, and creating an express lane should never occur as accidental side effects of exploratory gestures.
...
The following mockups were generated during the design exploration. They are illustrative rather than prescriptive: the typography, colors, geometry, and individual controls may change. Their purpose is to make the membrane, portal, and spatial-composition concepts concrete. They predate the conversation-level reuse policy and therefore omit its scope indicator and restriction states. The prose and notation in Sections~\ref{sec:reference-language}--\ref{sec:composition} govern those later refinements; these historical images are retained unchanged.

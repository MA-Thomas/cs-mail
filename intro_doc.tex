...
retained for historical context only. Its per-attempt bonds, direct recipient
collateral payouts, and funded user balances describe the earlier model, not the
current design. Current documents use one charge per relationship request,
request-linked refunds, and a company-owned member-distribution pool. The current
protocol specification and C-SQD financial-program profile take precedence.}}
\end{center}
...
\section{Historical Reference Settlement Semantics}

The earlier draft proposed the following rules. They are historical and are
not conformance requirements for the current request model.

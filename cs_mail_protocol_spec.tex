...
delivery. It \MUSTNOT{} contain plaintext content, legal identity evidence,
unrelated protocol identities, an account balance, or relationship history.
...
sender and recipient references, purpose or message class, validity interval,
maximum count or rate, capability version, and revocation conditions. Outside the
endpoint and routing boundary, those references \MUST{} be pairwise opaque.

An advertised express-lane capability \MUST{} contain a recipient signature, a
hard validity interval, and an enforceable maximum count or rate. Purpose labels
\MUSTNOT{} be represented as provider-enforced content policy when content is
end-to-end encrypted. A native opaque pair handle identifies the subject but is
not authority without the signed grant.
...
safe or that a look-alike domain belongs to an expected organization. A legacy
...
A message matching a valid capability \MAY{} be admitted without a bond while the
relationship remains unaccepted. Capability validation \MUST{} be deterministic,
auditable, scoped to the represented authority, and replay-safe. A capability
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item add signed identities, endpoint content encryption, key transparency,
          quotes, admission receipts, and decision receipts;

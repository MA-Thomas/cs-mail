...
\author{C-SQD}
\date{Draft 0.7 --- 25 September 2026}
...
annual service and member-cycle experience, and open enrollment. cs-mail's core is
also required by the future Phoros Account, so user testing informs design rather
than deciding whether development continues. Open enrollment introduces no real
redeemable value.
\end{abstract}
...
\section{What Changed in Draft 0.7}

\begin{itemize}
    \item There are no invitations and no invite-only pilot. Enrollment is open
    to anyone who completes a fresh login and bank-ownership verification; the
    one-person-one-account rules are the only gate. Milestone~7 becomes an
    open-enrollment launch with simulated value.
    \item Request pricing is redesigned as Milestone~1b, ahead of Milestone~2: one
    cs-mail-wide operator policy (processing component $C$ and collateral bounds)
    and up to eight request classes per protocol identity, published
    independently of any relationship. See
    \href{docs/request-pricing-design.md}{\texttt{docs/request-pricing-design.md}}.
\end{itemize}
...
\section{The Product and Its Users}
...
\paragraph{Enrollment.} Open to anyone who completes a fresh login and
bank-ownership verification. There are no invitations. The one-person-one-account
rules are the only gate: one cs-mail login per identity subject, one account per
verified bank token, and one member per bank-attested person. The service is run
by CSQD, the only provider, in one region on one PostgreSQL deployment. The
intended recipients are people who receive substantial unsolicited contact;
senders are people trying to reach them, including guests once the request page
ships.
...
\paragraph{Out of scope for this plan.} Independent-provider federation,
general email hosting (members emailing arbitrary external addresses), live
payments, mobile releases, and multi-region operation. Email is in
...
snapshots. Retention policy is explicit before user data exists.
...
evidence for a claim. At each handoff, run the local quality gate, which reports
the identity source manifest, dependency direction, rustfmt, Clippy, workspace
tests, PostgreSQL suites, and application binaries separately:
\begin{lstlisting}
CS_MAIL_TEST_DATABASE_URL=... scripts/quality-gate.sh
\end{lstlisting}
...
1 & Service and identity boot & Clean start, restart, open enrollment, simulator scenarios \\
1b & Request pricing & Operator policy with collateral bounds; per-address classes \\
...
7 & Open enrollment & Signed builds, recovery, restore, launch findings \\
...
\textbf{Decided (25 September 2026):} the fresh-database policy ends when the
first user data is created, at the first open enrollment. Until then a schema change may replace or renumber
existing migrations and requires a fresh database. From then on, migrations in
cs-mail and identity-model are forward-only: a merged migration is never edited,
...
\textbf{Status: complete (25 September 2026).} The three binaries report their
version and help. \texttt{scripts/quality-gate.sh} runs the identity source
manifest, dependency direction, rustfmt, Clippy, workspace tests, PostgreSQL
suites, and binary checks, each reported separately; see
\href{apps/README.md}{\texttt{apps/README.md}} for the boundary rules.

\section{Milestone 1: The Service and Identity Boot}

The operator can migrate an empty database, bootstrap CSQD as the provider
idempotently, start the API, publish service offers, and select simulator
scenarios---without editing SQL. Deployment-level state (configuration, accounts,
billing, and, from Milestone~1b, request pricing) is reached through a
deployment-scoped storage handle, not a relationship-scoped engine. Use Tokio, Axum, Rustls,
...
simulated bank-ownership evidence from development support code. Enrollment is
open: a fresh login and bank-ownership verification are the only requirements,
and the one-person-one-account rules are the only gate. The payment
...
\begin{claims}
\claim{PostgreSQL}{Repeating bootstrap cannot install a second provider signing key or replace the key registry.}
    {Two bootstrap runs, including one interrupted midway, leave two keys accepted as CSQD's or a changed registry.}
    {Two keys could then sign quotes and receipts as the provider.}
\claim{PostgreSQL}{Concurrent enrollments by the same person, through different logins or bank accounts, cannot create two accounts.}
    {Two enrollment operations for one bank-attested person both activate.}
...
\section{Milestone 1b: Request Pricing}

Replace the per-recipient pricing menu and relationship-scoped class publication
with the agreed design in
\href{docs/request-pricing-design.md}{\texttt{docs/request-pricing-design.md}}.
CSQD publishes one versioned operator policy---processing component $C$ and
cs-mail-wide collateral bounds---through the authenticated administration API.
Each protocol identity publishes up to eight request classes, each with collateral
within the bounds, signed by that address's own key and independent of any
relationship. A class outside the current bounds cannot be quoted until the
recipient republishes; issued quotes keep their terms. Quote prices become their
own value rather than fields of the policy snapshot. The replaced tables, APIs and
menu types are removed without aliases.

\begin{claims}
\claim{PostgreSQL}{A class outside the current bounds cannot be quoted, although its publication was valid when made.}
    {After the operator narrows the bounds, a quote is issued for a class outside them.}
    {Bounds that do not bind would let old classes escape policy indefinitely.}
\claim{PostgreSQL}{A publication signed for one address cannot be applied to another, including another address of the same account.}
    {A publication signed with address A's key changes address B's classes.}
    {Each address's classes are its own consent to paid approaches.}
\claim{PostgreSQL}{A quote issued concurrently with a republication or policy change carries the terms of exactly one publication and one policy version.}
    {An issued quote combines $C$, $S$, or a description from different versions.}
    {The charge and refund obligation must match what the sender was shown.}
\end{claims}

\textbf{Exit:} the operator changes the policy and a recipient's out-of-bounds
class stops being quotable until republished, from a clean database.

\textbf{Status: implemented (25 September 2026)}, with the deployment-scoped storage
handle; the three claims are challenged against PostgreSQL. Publishing the operator
policy moves behind the authenticated administration API in Milestone~1.
...
manages each address's request classes with collateral within the operator's bounds,
...
\section{Milestone 7: Open Enrollment With Simulated Value}
...
Open enrollment to the public, aiming first at recipients who receive substantial
unsolicited contact and senders who want to reach them, including guests if
Milestones~3b and~3c have shipped. Before live payments, establish that the
bank-attested person reference is stable across a person's bank accounts.
Configure the deployment domain's MX, SPF, DKIM, and DMARC records and monitor
outbound deliverability. Nondeveloper
testers use clean machines and the user guide without direct database repair.
...
usability findings, and a written summary of launch findings.

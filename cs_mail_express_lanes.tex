...
privacy semantics remain governed by that specification; the issuance, safety,
and lifetime rules sketched here will be defined normatively in a later
document.}}
\end{center}
...
post a refundable bond to reach you, and you accept or reject the relationship.
...
\textit{cs-mail} already has---bonded stranger and accepted person---do not fit
...
So we need a third permission state: a way to grant a chosen organization a
...
\textit{cs-mail} already gives us bonded first contact for strangers and the idea
...
skip the bond for a bounded purpose. Express lanes are that launch capability, given the
issuance, safety, and lifetime rules it needs to actually work.

\section{The Solution: Express Lanes}

An express lane is a recipient-granted, revocable, bond-free path for one sender
...
long}---not \emph{what} they say. Because \textit{cs-mail} content is end-to-end
encrypted, no provider reads a message to confirm it is ``really about your
account.'' ``Purpose'' is therefore a label, plus volume and time caps, plus your
power to revoke---not content inspection. This is a deliberate limit, and an
honest one.
...
floods you, or a lane is somehow misused---you revoke it, and the sender drops
back to the ordinary bonded gate, where it must pay to reach you again. Because
the cost of a mistake is a single tap, you can hand out lanes liberally without
building a permanent, unmanageable allowlist. A block always overrides a lane.

\medskip

The three permission states now cover the whole of your inbox:
...
\\
\midrule
A stranger & Posts a refundable bond; you accept or reject. & Automatic on any
first contact; settled by your decision. \\
A person you accept & Writes freely, no bond. & You accept the relationship; you
revoke it. \\
An organization you chose & Travels a bond-free express lane, bounded in purpose
...
quietly rerouted to the bonded gate. For these you choose a \emph{persistent}
...
Three constraints shape any implementation. Their details belong in a later
...
already where an incoming message is either admitted or sent to the bonded gate.
...
and the activity-anchored lifetime with its persistent option. This changes no
settlement or privacy meaning.

\section{Where This Leaves \textit{cs-mail}}

With express lanes, the permission model reads as one sentence: strangers post a
refundable bond to reach you, people you accept may write freely, and
